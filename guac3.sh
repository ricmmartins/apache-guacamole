...
# Check for MySQL connection and database existence
echo "Checking MySQL connection and database..."
if ! mysql -h "$MYSQL_HOST" -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" -e "USE $MYSQL_DB"; then
    echo "MySQL connection failed or database $MYSQL_DB does not exist. Please check the connection details."
    exit 1
fi
...
SCHEMA_EXISTS=$(mysql -h "$MYSQL_HOST" -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" -D "$MYSQL_DB" -e "SHOW TABLES LIKE 'guacamole_connection_group'" | grep -c 'guacamole_connection_group')
...
docker run --name guacamole --restart always -d --link guacd:guacd -e MYSQL_HOSTNAME="$MYSQL_HOST" \
    -e MYSQL_DATABASE="$MYSQL_DB" -e MYSQL_USER="$MYSQL_USER" -e MYSQL_PASSWORD="$MYSQL_PASSWORD" \
    -e GUACAMOLE_HOME="/etc/guacamole" -e GUACADMIN_PASSWORD="$GUACADMIN_PASSWORD" -p 8080:8080 $GUACAMOLE_IMAGE
